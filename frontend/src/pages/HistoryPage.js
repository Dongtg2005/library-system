...
const HistoryPage = () => {
  ...
  return (
    <div className="rounded-[2rem] border border-white/70 bg-white/85 p-6 page-fade dark:border-slate-800 dark:bg-slate-900/75">
      <h1 className="text-3xl font-black text-slate-950 dark:text-white">{t('historyPage.title')}</h1>
      <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">{t('historyPage.description')}</p>

      {loading ? (
        <div className="mt-6 rounded-2xl bg-slate-100 px-4 py-8 text-sm dark:bg-slate-800">{t('historyPage.loading')}</div>
      ) : (
          ...
          {history.map((item) => (
            <div key={item.id} className="rounded-3xl border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-950/80">
              ...
              <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                {t('myBorrows.borrowDate')}: {item.borrowDate || '-'} • {t('myBorrows.dueDate')}: {item.dueDate || '-'} • {t('myBorrows.returnDate')}: {item.returnDate || '-'}
              </p>
            </div>
          ))}
          {!history.length && (
            <div className="rounded-2xl bg-slate-100 px-4 py-8 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
