...
const HomePage = () => {
  ...
  const categories = useMemo(() => {
    const map = books.reduce((acc, book) => {
      const category = book.category || t('home.uncategorized');
    ...
    <div className="space-y-8 page-fade">
      <section className="grid gap-6 lg:grid-cols-[1.3fr_0.7fr]">
        <div className="rounded-[2rem] border border-white/70 bg-white/85 p-8 shadow-[0_30px_120px_rgba(15,23,42,0.08)] backdrop-blur dark:border-slate-800 dark:bg-slate-900/75">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            {!isAuthenticated && (
              <Link to="/register">
                <Button size="lg" variant="secondary">{t('home.joinNow')}</Button>
              </Link>
            )}
            {isAuthenticated && (
              <Link to="/my-books">
                <Button size="lg" variant="secondary">{t('home.myBooks')}</Button>
              </Link>
            )}
          </div>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-3xl bg-slate-100 p-4 dark:bg-slate-800/80">
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('home.catalogBooks')}</p>
              <p className="mt-2 text-3xl font-black">{books.length}</p>
            </div>
            <div className="rounded-3xl bg-slate-100 p-4 dark:bg-slate-800/80">
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('home.activeBorrows')}</p>
              <p className="mt-2 text-3xl font-black">{activeBorrows}</p>
            </div>
            <div className="rounded-3xl bg-slate-100 p-4 dark:bg-slate-800/80">
              <p className="text-sm text-slate-500 dark:text-slate-400">{t('home.overdue')}</p>
        ...
        <div className="rounded-[2rem] border border-white/70 bg-white/85 p-6 shadow-[0_20px_80px_rgba(15,23,42,0.08)] backdrop-blur dark:border-slate-800 dark:bg-slate-900/75">
          <p className="text-sm font-semibold uppercase tracking-[0.25em] text-slate-500 dark:text-slate-400">{t('home.search')}</p>
          ...
          <div className="mt-4 space-y-3">
            <Link to="/books" className="block rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold dark:bg-slate-800">{t('home.categoryFilter')}</Link>
            <Link to="/books" className="block rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold dark:bg-slate-800">{t('home.authorFilter')}</Link>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-[2rem] border border-white/70 bg-white/80 p-6 backdrop-blur dark:border-slate-800 dark:bg-slate-900/70">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-black text-slate-950 dark:text-white">{t('home.featuredBooks')}</h2>
          <Link to="/books" className="text-sm font-semibold text-primary">{t('home.viewAll')}</Link>
        </div>

        {loading ? (
          <div className="mt-6 rounded-3xl bg-slate-100 p-8 text-sm text-slate-500 dark:bg-slate-800">{t('home.loadingBooks')}</div>
        ) : (
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {featured.map((book) => (
              <article key={book.id} className="rounded-[1.5rem] border border-slate-200 bg-white p-5 transition hover:-translate-y-1 hover:shadow-lg dark:border-slate-800 dark:bg-slate-950/90">
                ...
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{book.status} · {book.availableQty}/{book.totalQuantity} {t('home.available')}</p>
                <Link to={`/books/${book.id}`} className="mt-4 inline-flex text-sm font-semibold text-primary">{t('home.viewDetail')}</Link>
              </article>
            ))}
            {!featured.length && (
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <div className="rounded-[2rem] border border-white/70 bg-white/80 p-6 dark:border-slate-800 dark:bg-slate-900/70">
          <h3 className="text-xl font-black text-slate-950 dark:text-white">{t('home.categories')}</h3>
          <div className="mt-4 space-y-3">
            {categories.map((category) => (
              <div key={category.name} className="flex items-center justify-between rounded-2xl bg-slate-100 px-4 py-3 dark:bg-slate-800">
                ...
                <span className="text-sm text-slate-500 dark:text-slate-400">{category.total} {t('home.booksCount')}</span>
              </div>
            ))}
            {!categories.length && (
              <div className="rounded-2xl bg-slate-100 px-4 py-6 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                {t('home.categoryDataInfo')}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="rounded-[2rem] border border-white/70 bg-white/80 p-6 dark:border-slate-800 dark:bg-slate-900/70">
          <h3 className="text-xl font-black text-slate-950 dark:text-white">{t('home.accountActivity')}</h3>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
            {isAuthenticated ? `${t('home.signedInAs')} ${user?.fullName || user?.email}` : t('home.signInPrompt')}
          </p>
          <div className="mt-4 space-y-3">
            {borrowHistory.slice(0, 4).map((item) => (
              <div key={item.id} className="rounded-2xl border border-slate-200 px-4 py-3 text-sm dark:border-slate-800">
                ...
                <p className="text-slate-500 dark:text-slate-400">{t('home.record')}: {item.id}</p>
              </div>
            ))}
            {isAuthenticated && !borrowHistory.length && (
              <div className="rounded-2xl bg-slate-100 px-4 py-6 text-sm text-slate-600 dark:bg-slate-800 dark:text-slate-300">
                {t('home.noBorrowHistory')}
              </div>
            )}
          </div>
        </div>
